...
df = spark.read \
    ...
    .option("query",
            f"SELECT json_data -> 'data' AS json FROM covid19_stage WHERE date_of_data = '{airflow_jinja_ds}'") \
    .option('customSchema', 'json STRING') \
    .load()
...
list_of_columns = ['col.date', 'col.confirmed', 'col.deaths', 'col.recovered', 'col.confirmed_diff', 'col.deaths_diff',
